...
function Navbar({name, appSetting, handleClickOnLang, handleClickOnTheme}: Props) {
    return (
        <nav>
            <div id="name">{name}</div>

            <div className="toggle-buttons">
                <button id="mode" onClick={handleClickOnTheme}>{appSetting.lightMode ? "Dark" : "Light"}</button>
                <button id="style" onClick={handleClickOnTheme}>Style</button>
                <button id="DE" className="language-icon" onClick={handleClickOnLang}>
